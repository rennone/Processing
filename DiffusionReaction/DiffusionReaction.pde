...
void reset() {
  w = width/res; //res: size of a pixel
  h = height/res;
  s = w*h;
  img = createImage(w, h, RGB); //create w*h pixel image format RGB
  pat = new float[s];
  // random init
  for(int i=0; i<s; i++) 
...
void draw() {
  ...
  if(border) {
    mouseX = constrain(mouseX,0,width-1);  //constrol not to over limit
    mouseY = constrain(mouseY,0,height-1); //
  }
  
  // add a circular drop of chemical
  //rdrop radius of drop circle
  if(mousePressed) {
      if(mouseButton != CENTER) {
      int x0 = mod((mouseX-dx)/res, w); //get click point
      ...
      for(int y=y0-r; y<y0+r;y++)
        for(int x=x0-r; x<x0+r;x++) {
          ...
          if(dist(x,y,x0,y0) < r)
            pat[xwrap+w*ywrap] = mouseButton == LEFT ? 255 : 0; //left click 255, right click 0
        }
    }
  }
  ...
  img.loadPixels(); //make possible to access img.pixels
  for(int x=0; x<w; x++)
    for(int y=0; y<h; y++) {
      int c = (x+dx/res)%w + ((y+dy/res)%h)*w;  //res => zoom?
      int i = x+y*w;
  ...
  image(img, 0, 0, res*w, res*h); // when res>1 protorude = zoom 
...
void keyPressed() {
  switch(key) {
    ...
    case '+': lim = min(lim+8, 255); println(lim);break;
    case '-': lim = max(lim-8, 0); println(lim);break;
    case CODED:
      switch(keyCode) {
...
void mouseDragged() {
  if(mouseButton == CENTER && !border) {
    dx = mod(dx + mouseX - pmouseX, width); //mouseX now, pmoseX: prev
  ...
  for(int i=0; i<scl; i++) {
    ...
    for(int j=0; j<dirs; j++) {
      ...
      for(int l=0; l<s; l++) { 
        ...
        if(x1<0){
          if(border) continue; else x1 = w-1-(-x1-1)% w;
        }
        else if(x1>=w) {
          if(border) continue; else x1 = x1%w;
        }
        if(y1<0) {
          if(border) continue; else y1 = h-1-(-y1-1)% h; 
        }
        else if(y1>=h){
          if(border) continue; else y1 = y1%h;
        }
        // update median
